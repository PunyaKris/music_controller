...
def spotify_callback(request, format=None):
    ...
    # FIX: Check if we actually got an access_token before saving
    if not access_token:
        # If this happens, Spotify returned an error (likely 'invalid_grant')
        return redirect('/')
...
class CurrentSong(APIView):
    def get(self, request, format=None):
        ...
        if 'item' not in response or response['item'] is None:
            return Response({"message": "No song playing"}, status=status.HTTP_200_OK)
